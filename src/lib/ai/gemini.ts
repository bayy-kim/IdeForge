...
/**
 * Wraps fetch with a hard timeout. Without this, a hanging/slow provider
 * response can silently eat the entire serverless function budget (up to
 * Vercel's 300s ceiling) instead of failing fast so retry/fallback logic
 * can kick in.
 */
...
async function callGemini(
  ...
  for (const model of modelsToTry) {
    ...
    while (attempt < maxAttempts) {
      ...
      let res: Response;
      try {
        res = await fetchWithTimeout(geminiUrlFor(model), {
          method: "POST",
          headers: {
        ...
        if (isTimeoutError(err) && attempt < maxAttempts) {
          continue; // retry same model once more before giving up on it
        }
        if (isTimeoutError(err)) {
          lastRetiredModel = model; // reuse for messaging; break to try next model
          break;
        }
...
async function callClaude(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      throw new GeminiRequestError(
        `Gagal menghubungi Claude API: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
...
async function callCustom(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      throw new GeminiRequestError(
        `Gagal menghubungi Custom API: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
